...
let argv = yargs(hideBin(process.argv))
  ...
  .option("cn", {
    ...
    default: 0,
    describe:
      "Number of common node to initialize the testnet with (default 0)",
    type: "number",
  })
  .option("s", {
    alias: "start",
...
broadcast-mode = "sync"
`;
const scriptStop = path.join(
  nodesDir,
  platform == "win32" ? "stopAll.vbs" : "stopAll.sh"
);
const scriptStart = path.join(
  nodesDir,
  platform == "win32" ? "startAll.vbs" : "startAll.sh"
);
...
const main = async function () {
  ...
  try {
    ...
    const { app, tendermint, preMinePerAccount, preMineAccounts, privateKeys } =
      config;
    if (app.chain_id) {
    ...
    const daemonApp =
      platform == "win32" ? `${app.chain_name}d.exe` : `${app.chain_name}d`;
    ...
    if (stop) {
      if (stop === "all") {
        ...
        for (let i = 0; i < argv.length; i++) {
          ...
          const { stdout, stderr } = await execPromis(script, {
            cwd: nodesDir,
          });
    ...
    if (init || isKeep) {
      ...
      const nodeKey = {
        priv_key: {
          ...
          value:
            "ggl/VtKrHieAP4T2SiOvZcmVyAoU223akLFh9yAfHrgFRZT3YVRQxhCH95PMkXvA5gCZhJU7Yr63f94BO69mbg==",
        },
      };
      const privValidatorConsensusKey = {
        address: "0176D23075D7281F0801EE93A249277BC467CBC0",
        pub_key: {
          type: "tendermint/PubKeyEd25519",
          value: "YJcd/+duDs3rxJ1PkwbC5BQytO6yK60RC0XVHA594ck=",
        },
        priv_key: {
          type: "tendermint/PrivKeyEd25519",
          value:
            "5rimRtjOgBEylz1uyY3xuhFWc00oEsXV08J5Tvw781tglx3/524OzevEnU+TBsLkFDK07rIrrRELRdUcDn3hyQ==",
        },
      };
      const createValidator = {
          ...
          messages: [
            {
              ...
              delegator_address: "0xbf657D0ef7b48167657A703Ed8Fd063F075246D7",
              validator_address: "0xbf657D0ef7b48167657A703Ed8Fd063F075246D7",
              pubkey: {
              ...
              bls_key:
                "a547a187ae6925835edc80790611f3d2376a42314f653e1a73db9650427afd1714e6bba474fe2618f8a47f6d5d7506c9",
              bls_proof:
                "9034144d203a42fbf61993d194b8149ea00afbec3a6ce75eedc0a55023f1ae07ca4305d8f43d01d0b772bce9a356fbab1382826ceb01d33de9c4e093a615d396f350a77a5e24ac1ede74cbe498da648bdefdf01d50d557232bd02e56a7b6fb15",
            },
          ],
          memo: "4fd5bef2f67258e381fe1c5632f6d9b99531610d@192.168.0.1:26656",
          timeout_height: "0",
          extension_options: [],
        ...
        signatures: [
          "LHWs3p4h0sXl93qO81g8o0NvvfBEbujdvvW/XPd8/2NuonUndBEfZSXFB3x+ukv4hjlUU4hO2/vvB270vKfa/wE=",
        ],
      };
      ...
      if (!isKeep) {
...

        {
          ...
          if (commonNode > 0) {
            ...
            const genesisPath = path.join(
              nodesDir,
              `node0/${daemon}/config/genesis.json`
            );
            for (let i = validators; i < nodesCount; i++) {
              await fs.copy(
                genesisPath,
                path.join(nodesDir, `node${i}/${daemon}/config/genesis.json`)
              );
            }
          }
          ...
          if (fixedFirstValidator) {
            await fs.writeJSON(
              path.join(nodesDir, `node0/${daemon}/config/node_key.json`),
              nodeKey
            );
            await fs.writeJSON(
              path.join(
                nodesDir,
                `node0/${daemon}/config/priv_validator_key.json`
              ),
              privValidatorConsensusKey
            );
            await fs.outputJSON(
              path.join(nodesDir, `node0/${daemon}/key_seed.json`),
              keySeed
            );
            const keyringPath = path.join(
              nodesDir,
              `node0/${daemon}/keyring-test`
            );
            await fs.emptyDir(keyringPath);
            await fs.writeFile(
              path.join(
                keyringPath,
                `bf657d0ef7b48167657a703ed8fd063f075246d7.address`
              ),
        ...
        for (let i = 0; i < nodesCount; i++) {
          const nodeKey = await fs.readJSON(
            path.join(nodesDir, `node${i}/${daemon}/config/node_key.json`)
          );
          const nodeId = privKeyToBurrowAddres(nodeKey.priv_key.value);
          nodeIds.push(nodeId);

          const keySeedPath = path.join(
            nodesDir,
        ...
        for (let i = 0; i < nodesCount; i++) {
          ...
          if (Array.isArray(preMineAccounts)) {
            ...
            for (const ac of preMineAccounts) {
              ...
              balances.push(
                Object.assign(JSON.parse(JSON.stringify(balance)), { address })
              );
            }
          }
          ...
          const genesisPath = path.join(
            nodesDir,
            `node${i}/${daemon}/config/genesis.json`
          );
          ...
          if (fixedFirstValidator) {
            appState.auth.accounts[0].base_account.address = keySeed.address;
            appState.bank.balances[0].address = keySeed.address;
        ...
        for (let i = 0; i < nodesCount; i++) {
          ...
          const configPath = path.join(
            nodesDir,
            `node${i}/${daemon}/config/config.toml`
          );
          ...
          const clientConfigPath = path.join(
            nodesDir,
            `node${i}/${daemon}/config/client.toml`
          );
          data = clientCfg;
  ...
  if [[ -n $pid ]]; then kill -15 $pid; fi`;
          let startPath = path.join(
            nodesDir,
            `start${i}.` + (platform == "win32" ? "bat" : "sh")
          );
          let stopPath = path.join(
            nodesDir,
            `stop${i}.` + (platform == "win32" ? "bat" : "sh")
          );
        ...
        let stopAllPath = path.join(
          nodesDir,
          `stopAll.` + (platform == "win32" ? "vbs" : "sh")
        );
    ...
    if (start && start.toLowerCase() !== "no") {
      if (start === "all") {
        ...
        const script = path.join(
          nodesDir,
          `startTemp.` + (platform == "win32" ? "vbs" : "sh")
        );
