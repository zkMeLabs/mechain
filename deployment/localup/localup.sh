...
function init() {
	...
	for ((i = 0; i < ${size}; i++)); do
		...
		if [ "$i" -eq 0 ]; then
			...
			${bin} keys import validator0 ${validator0_prikey} --secp256k1-private-key --keyring-backend test --home ${local_env}/validator0
			${bin} keys import relayer0 ${relayer0_prikey} --secp256k1-private-key --keyring-backend test --home ${local_env}/relayer0
			${bin} keys show devaccount --keyring-backend test --home ${local_env}/validator0 >${local_env}/validator0/devaccount_info 2>&1
			${bin} keys show validator0 --keyring-backend test --home ${local_env}/validator0 >${local_env}/validator0/info 2>&1
			${bin} keys show relayer0 --keyring-backend test --home ${local_env}/relayer0 >${local_env}/relayer0/relayer_info 2>&1
		else
	...
	for ((i = 0; i < ${sp_size}; i++)); do
		...
		if [ "$i" -eq 0 ]; then
			${bin} keys import sp0 ${sp0_prikey} --secp256k1-private-key --keyring-backend test --home ${local_env}/sp${i}
			${bin} keys show sp0 --keyring-backend test --home ${local_env}/sp${i} >${local_env}/sp${i}/info 2>&1
		else
...
function start() {
	size=$1
	for ((i = 0; i < ${size}; i++)); do
		mkdir -p ${local_env}/validator${i}/logs
		nohup "${bin}" start --home ${local_env}/validator${i} \
			--keyring-backend test \
			...
			--rpc.unsafe true \
			--log_format json >${local_env}/validator${i}/logs/node.log &
	done
}
