...
function generate_genesis() {
	...
	for ((i = 0; i < ${size}; i++)); do
		...
		sed -i -e "s/\"redundant_data_chunk_num\": 4/\"redundant_data_chunk_num\": 1/g" "${workspace}"/.local/validator${i}/config/genesis.json
		sed -i -e "s/\"redundant_parity_chunk_num\": 2/\"redundant_parity_chunk_num\": 1/g" "${workspace}"/.local/validator${i}/config/genesis.json
