...
// IStorageSetTagsIterator is returned from FilterSetTags and is used to iterate over the raw logs and unpacked data for SetTags events raised by the IStorage contract.
type IStorageSetTagsIterator struct {
	Event *IStorageSetTags // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *IStorageSetTagsIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *IStorageSetTagsIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *IStorageSetTagsIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// IStorageSetTags represents a SetTags event raised by the IStorage contract.
type IStorageSetTags struct {
	Creator common.Address
	Raw     types.Log // Blockchain specific contextual infos
}

// FilterSetTags is a free log retrieval operation binding the contract event 0x0dfa4d213622a2d45fdb1ae13ccc9aeb24c1e4013288cd2a5eae30f497d2e568.
//
// Solidity: event SetTags(address indexed creator)
func (_IStorage *IStorageFilterer) FilterSetTags(opts *bind.FilterOpts, creator []common.Address) (*IStorageSetTagsIterator, error) {
	...
	logs, sub, err := _IStorage.contract.FilterLogs(opts, "SetTags", creatorRule)
	if err != nil {
		return nil, err
	}
	return &IStorageSetTagsIterator{contract: _IStorage.contract, event: "SetTags", logs: logs, sub: sub}, nil
}

// WatchSetTags is a free log subscription operation binding the contract event 0x0dfa4d213622a2d45fdb1ae13ccc9aeb24c1e4013288cd2a5eae30f497d2e568.
//
// Solidity: event SetTags(address indexed creator)
func (_IStorage *IStorageFilterer) WatchSetTags(opts *bind.WatchOpts, sink chan<- *IStorageSetTags, creator []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _IStorage.contract.WatchLogs(opts, "SetTags", creatorRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(IStorageSetTags)
				if err := _IStorage.contract.UnpackLog(event, "SetTags", log); err != nil {
					return err
				}
				event.Raw = log
...
// ParseSetTags is a log parse operation binding the contract event 0x0dfa4d213622a2d45fdb1ae13ccc9aeb24c1e4013288cd2a5eae30f497d2e568.
//
// Solidity: event SetTags(address indexed creator)
func (_IStorage *IStorageFilterer) ParseSetTags(log types.Log) (*IStorageSetTags, error) {
	event := new(IStorageSetTags)
	if err := _IStorage.contract.UnpackLog(event, "SetTags", log); err != nil {
		return nil, err
	}
