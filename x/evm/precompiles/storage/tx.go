...
func (c *Contract) registerTx() {
	...
	c.registerMethod(SetTagsMethodName, 60_000, c.SetTags, "SetTags")
...
func (c *Contract) SetTags(ctx sdk.Context, evm *vm.EVM, contract *vm.Contract, readonly bool) ([]byte, error) {
	if readonly {
		return nil, errors.New("set tag for group method readonly")
	}
	method := GetAbiMethod(SetTagsMethodName)
	var args SetTagsArgs
	if err := types.ParseMethodArgs(method, &args, contract.Input[4:]); err != nil {
		return nil, err
	}
	...
	if err := c.AddLog(
		evm,
		GetAbiEvent(c.events[SetTagsMethodName]),
		[]common.Hash{common.BytesToHash(contract.Caller().Bytes())},
	); err != nil {
		return nil, err
	}
