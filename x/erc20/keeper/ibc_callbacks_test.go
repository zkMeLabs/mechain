...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	ethPk, err := ethsecp256k1.GenPrivKey()
...
func (suite *KeeperTestSuite) TestConvertCoinToERC20FromPacket() {
	senderAddr := "mc1x2w87cvt5mqjncav4lxy8yfreynn273x3sy9tq"
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() transfertypes.FungibleTokenPacketData {
				...
				suite.Require().NoError(err)
				err = suite.app.BankKeeper.SendCoinsFromModuleToAccount(suite.ctx, evmtypes.ModuleName, sender, coins)
				suite.Require().NoError(err)
